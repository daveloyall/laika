...
Finally, there is an adjustment to the source code for the paths in three getResource calls.  You will need to checkout the source code and change the following two files:

jpartlow@fook:~/dev/osourcery/elbe/ccrvalidator/ValidationService$ svn diff
Index: src/org/openhealthdata/validator/ValidationManager.java
===================================================================
--- src/org/openhealthdata/validator/ValidationManager.java	(revision 3)
+++ src/org/openhealthdata/validator/ValidationManager.java	(working copy)
@@ -77,8 +77,8 @@
 		KnowledgeBuilder builder = KnowledgeBuilderFactory
 				.newKnowledgeBuilder();
 		// Get Rule directory
-		URL c = this.getClass().getClassLoader().getResource("/org/astm/ccr/rules/core");
-		URL v1 = this.getClass().getClassLoader().getResource("/org/astm/ccr/rules/v1");
+		URL c = this.getClass().getClassLoader().getResource("org/astm/ccr/rules/core");
+		URL v1 = this.getClass().getClassLoader().getResource("org/astm/ccr/rules/v1");
 
 		File coreDir = new File(c.toURI());
 		File v1Dir = new File(v1.toURI());
Index: src/org/openhealthdata/validation/CCRV1SchemaValidator.java
===================================================================
--- src/org/openhealthdata/validation/CCRV1SchemaValidator.java	(revision 3)
+++ src/org/openhealthdata/validation/CCRV1SchemaValidator.java	(working copy)
@@ -88,7 +88,7 @@
 		// create Validator
 		// create a SchemaFactory capable of understanding WXS schemas
 	    SchemaFactory factory = SchemaFactory.newInstance(XMLConstants.W3C_XML_SCHEMA_NS_URI);
-	    URL xsdURL = this.getClass().getClassLoader().getResource("/org/openhealthdata/validation/CCRV1.xsd");
+	    URL xsdURL = this.getClass().getClassLoader().getResource("org/openhealthdata/validation/CCRV1.xsd");
 	    File xsdFile = null;
 		try {
 			xsdFile = new File(xsdURL.toURI());

And then compile them with javac into your vendor/ccr-validation-service binaries.  The following command assumes the following structure:

./laika/vendor/ccr-validation-service/
./ccrvalidator/ValidationService/

and assumes that the command is being run from the ccrvalidator/ValidationService/ directory.  Modify the pathing as needed for your layout.  The key point is that the binary packages lib jars and classes dir must be in the class path.  The destination directory must the binary classes dir, and you need to compile the indicated files after making the changes noted above.

jpartlow@fook:~/dev/osourcery/elbe/ccrvalidator/ValidationService$ javac -verbose -cp '../../laika/vendor/ccr-validation-service/WEB-INF/lib/*:../../laika/vendor/ccr-validation-service/WEB-INF/classes' -d ../../laika/vendor/ccr-validation-service/WEB-INF/classes/ src/org/openhealthdata/validation/CCRV1SchemaValidator.java src/org/openhealthdata/validator/ValidationManager.java
